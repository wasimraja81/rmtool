...
Requires astroquery -- run scripts/setup_venv.sh once to create
~/venv/rmtool with this project's full Python dependency set
(requirements.txt), then use ~/venv/rmtool/bin/python3 to run this
script.
...
possible after that first interactive use. If the OS keyring itself is
unreachable (e.g. no secret-service backend running -- common on headless
machines), a bounded probe detects that before login() and falls back to
prompting every run instead of hanging indefinitely with no message.
...
try:
    ...
    print(
        "This script needs this project's Python venv -- run "
        "scripts/setup_venv.sh once, then use "
        "~/venv/rmtool/bin/python3 to run this script.",
        file=sys.stderr,
    )
    sys.exit(1)

logger = logging.getLogger("casda_fetch")


def setup_logging(outdir: str) -> str:
    """One log file per run, under outdir, timestamped: the script's own
    narrative (everything printed to the console) plus every HTTP
    request CASDA/astroquery issues, for provenance. Returns the log
    file's path.

    Console output is unchanged from a plain logger.info()-based script (bare
    message text, INFO to stdout, ERROR to stderr) -- only the file
    handler carries timestamps/levels, and only the file handler receives
    the HTTP-request records logged at DEBUG (see log_request() below)."""
    ...
    log_path = os.path.join(outdir, f"casda_fetch_{timestamp}.log")
...
_JOB_CREATED_URL = re.compile(r"/async/[0-9a-fA-F-]+$")
# The job-creation endpoint itself -- .../async, optionally with a query
# string (?ID=...), but critically NOT .../async/<uuid>/parameters or
# .../async/<uuid>/phase, which also redirect through the same bare job
# URL and would otherwise be mistaken for a second creation event.
_JOB_CREATE_REQUEST_URL = re.compile(r"/async(\?|$)")

# Staging job URLs created so far that haven't reached a terminal phase yet.
# casda.cutout() blocks until every job it creates reaches COMPLETED before
# returning, so the fetch loop clears this right after each successful
# cutout() call -- what's left in here at any moment is still live on
# CASDA's servers, never something already finished.
_pending_job_urls: list[str] = []

# Set on a response hook call that saw the creation POST redirect (a 3xx);
# consumed on the VERY NEXT hook call, which is the redirect's target and
# carries the job URL. Needed because a 303 (confirmed via CASDA responses,
# 2026-09-21) makes `requests` reissue the redirected request as GET, per
# HTTP semantics -- so the response with the job's own URL never has
# method=="POST", and response.history is only populated on the object
# session.post() finally returns, not on any response as this hook sees it
# mid-redirect (confirmed directly, not assumed) -- so tracking state
# across consecutive hook calls, in the order they're invoked, is the only
# reliable way to associate the two.
_awaiting_job_creation_redirect = False


def abort_pending_jobs() -> None:
    """Cancel every currently-live staging job, e.g. on Ctrl+C. Doesn't
    need a fresh OPAL username/password -- a job's own URL is itself
    what CASDA checks to authorize cancelling it, the same as the hint
    printed when each job was created.

    ABORT is not guaranteed to succeed -- confirmed directly
    (2026-09-21): CASDA correctly rejects it (HTTP 400) for a job in
    SUSPENDED phase, per the IVOA UWS 1.1 spec's own state diagram --
    client-initiated ABORT is only a valid transition from
    PENDING/QUEUED/EXECUTING. SUSPENDED is a normal, system-driven pause
    (temporary resource contention, spec section 2.1.3) that resolves on
    its own back to EXECUTING -- not a fault to work around by trying a
    different HTTP method. When ABORT fails, this reports it plainly
    rather than claiming success, since the job may still complete on
    CASDA's servers regardless of what was asked here."""
    if not _pending_job_urls:
        return
    logger.info(
        f"\nInterrupted -- cancelling {len(_pending_job_urls)} staging "
        f"job(s) issued so far..."
    )
    failed_urls = []
    for job_url in list(_pending_job_urls):
        logger.info(f"  aborting {job_url} ...")
        try:
            resp = requests.post(
                f"{job_url}/phase", data={"phase": "ABORT"}, timeout=30)
            if resp.status_code < 400:
                logger.info(f"    -> HTTP {resp.status_code} (cancelled)")
            else:
                logger.info(
                    f"    -> HTTP {resp.status_code}: {resp.text[:500]}")
                failed_urls.append(job_url)
        except requests.RequestException as exc:
            logger.info(f"    -> failed to send the cancel request: {exc}")
            failed_urls.append(job_url)

    n_cancelled = len(_pending_job_urls) - len(failed_urls)
    if n_cancelled:
        logger.info(f"Cancelled {n_cancelled} job(s). Nothing was downloaded.")
    if failed_urls:
        logger.info(
            f"WARNING: could not cancel {len(failed_urls)} job(s) -- "
            f"they may still be running on CASDA's servers and could "
            f"complete with data. This is expected if CASDA had "
            f"paused a job (SUSPENDED) right when Ctrl+C was sent -- "
            f"that phase can't be cancelled directly and resolves on "
            f"its own; wait for it to resume, then retry: python3 "
            f"{os.path.basename(__file__)} --abort <url>, or check/"
            f"retrieve it another way. Job(s) still possibly live:"
        )
        for job_url in failed_urls:
            logger.info(f"  {job_url}")
    _pending_job_urls.clear()


def log_request(response, *args, **kwargs):
    """requests 'response' hook, registered on every session CASDA-facing
    code in this script uses (see register_request_logging()) -- records
    every HTTP request for provenance: method, full URL, status,
    and body.

    Deliberately reads ONLY response.request.method/url/body and
    response.status_code -- NEVER response.request.headers or
    response.headers. HTTP Basic Auth (astroquery's login() and its
    per-file DataLink lookups both use auth=(username, password)) puts
    the password, base64-encoded -- not encrypted, trivially reversible
    -- into an Authorization request header. This function's own code
    never reading .headers at all is what makes it safe to leave this
    enabled on every run, auditable by inspection
    rather than by trusting a log-level setting: there is no code path
    here that can write a credential to disk, not just a filter that
    happens to be configured not to today."""
    ...
    # Job creation is a POST to .../async(?ID=...); confirmed directly
    # (2026-09-21 log) that CASDA answers this with a 303, and the
    # response carrying the new job's bare .../async/<uuid> URL is
    # therefore a SEPARATE, later hook call with method=="GET" -- see
    # _awaiting_job_creation_redirect's own comment for why state has to
    # be tracked across calls rather than read off one response.
    global _awaiting_job_creation_redirect
    if (req.method == "POST" and _JOB_CREATE_REQUEST_URL.search(req.url)
            and 300 <= response.status_code < 400):
        _awaiting_job_creation_redirect = True
    elif _awaiting_job_creation_redirect:
        _awaiting_job_creation_redirect = False
        if _JOB_CREATED_URL.search(response.url):
            _pending_job_urls.append(response.url)
            logger.info(
                f"  Staging job created: {response.url}\n"
                f"  To cancel it by hand later (e.g. if this process gets "
                f"killed outright rather than interrupted -- Ctrl+C already "
                f"cancels any job still running automatically): "
                f"python3 {os.path.basename(__file__)} --abort {response.url}"
            )
    return response


def register_request_logging(session) -> None:
    """Attach log_request() to a requests.Session's own response hooks,
    alongside whatever astroquery already registered there (BaseQuery.__init__
    does the same via self._session.hooks['response'].append(...)) --
    appended, not replacing astroquery's own hook."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--abort", nargs="+", metavar="JOB_URL", default=None,
        help="Cancel one or more staging jobs by URL (as printed when each "
             "job was created, or seen in a run's own log file) and exit -- "
             "does not need --username/--target/anything else below, or a "
             "fresh OPAL password: a job's own URL is itself what CASDA "
             "checks to authorize cancelling it.",
    )
    parser.add_argument(
        "--username",
        help="OPAL account email (password is read from the OS keyring -- "
             "see the password-handling note in this script's own module "
             "docstring). Required unless --abort is given.",
    )
    ...
    if not args.username:
        parser.error("--username is required (unless --abort is given)")
    if not args.target and args.ra is None:
        parser.error("one of --target or --ra is required (unless --abort is given)")
...
def resolve_target(args: argparse.Namespace) -> SkyCoord:
    if args.target:
        if args.target in PRESET_TARGETS:
            ...
            logger.info(f"'{args.target}' matched this project's own preset "
                  f"list -> resolving '{name}' (internal lookup)...")
            return SkyCoord.from_name(name)
        logger.info(f"'{args.target}' is not one of this project's own presets "
...
def select_iquv_mask(rows, version: Optional[int] = None) -> list:
    ...
    if not (has_conv("q") and has_conv("u")):
        logger.info(f"    ERROR: Q and/or U lack a .conv. variant -- this whole "
    ...
    def pick_version(idxs: list, letter: str) -> int:
        ...
        if version is not None:
            ...
            logger.info(f"    Stokes {letter.upper()}: requested v{version} not "
    ...
    for letter in STOKES_LETTERS:
        ...
        if not idxs:
            logger.info(f"    Stokes {letter.upper()}: no file found for this observation")
            continue
        ...
        if conv_idxs:
            ...
            logger.info(f"    Stokes {letter.upper()}: {len(idxs)} file(s) matched, "
            ...
            logger.info(f"    WARNING: Stokes {letter.upper()} has no .conv. "
...
def resolve_datalink_sync_url(datalink_url: str, auth) -> Optional[str]:
    ...
    with semantics='#this' and a sync endpoint, not the
...
def fetch_and_curate_beamlog(obs_id: str, auth, obs_dir: str) -> Optional[str]:
    ...
    if sbid is None:
        logger.info(f"    could not parse an SBID number out of {obs_id!r} -- "
              f"skipping beamlog fetch")
        return None
    try:
        logger.info(f"    looking up {obs_id}'s diagnostics evaluation file...")
        datalink_url = find_diagnostics_datalink_url(sbid)
        if datalink_url is None:
            logger.info(f"    no diagnostics evaluation file published for "
                  f"SB{sbid} -- cannot curate a beamlog for it")
            return None
        sync_url = resolve_datalink_sync_url(datalink_url, auth)
        if sync_url is None:
            logger.info(f"    could not resolve a download URL from the "
                  f"diagnostics file's DataLink response -- cannot "
                  f"curate a beamlog")
            return None
        logger.info(f"    streaming the diagnostics tarball for its beamlog "
              f"(this can take a few minutes -- stops as soon as a "
              f"match is found)...")
        raw_text = fetch_beamlog_text(sync_url, sbid)
        if raw_text is None:
            logger.info(f"    no matching beamlog found in the diagnostics "
                  f"tarball for SB{sbid}")
            return None
    except Exception as exc:
        logger.info(f"    FAILED to fetch beamlog: {type(exc).__name__}: {exc}")
        return None

    out_path = os.path.join(obs_dir, "beamlog_target.txt")
    n = curate_beamlog(raw_text, out_path)
    logger.info(f"    wrote {out_path} ({n} channels) -- pass this as "
...
def filter_target_coverage(observations: list, coord: SkyCoord,
    ...
    for obs in observations:
        if observation_covers_target(obs, coord, cutout_radius_arcmin):
            ...
            logger.info(f"  excluding {obs.obs_id}: field footprint doesn't "
...
def print_band_report(bands: list, n_observations_total: int) -> None:
    ...
    if not bands:
        logger.info("No band with both Q and U present as .conv -- this "
              "position isn't usable yet with what CASDA currently has "
              "public.")
        return
    if len(bands) >= 2:
        logger.info("Multi-band tomography looks possible.")
    ...
    for band in bands:
        ...
        for i, obs in enumerate(band.observations):
            ...
            logger.info(f"  SBID {sbid} (observed {date_str}){default_tag}")
            for (letter, is_conv), versions in sorted(obs.file_versions.items()):
                if len(versions) > 1:
                    ...
                    logger.info(f"      Stokes {letter.upper()} {conv_str} has "
...
def default_omp_threads() -> int:
    """rmsynth_omp_threads= value for the generated pipeline cfg --
    caps at 6 (this project's own no-hyperthreading convention, see
    run_pipeline.sh's own DEFAULT_OMP_NUM_THREADS) when enough cores
    exist, but never asks for more than are available on whatever host
    this generated cfg eventually runs on (which may not even be the
    machine casda_fetch.py itself runs on). OpenMP does not do this
    clamping on its own: confirmed directly that an OMP_NUM_THREADS
    higher than the process's available CPU affinity still runs with
    no error, just via oversubscription -- every requested thread
    time-sliced onto too few cores instead of one thread per core,
    measurable overhead on a small host, not a crash.
    sched_getaffinity (Linux-only, matches this project's own
    platform) is used over a bare os.cpu_count() since it reflects
    constraints os.cpu_count() alone can't see -- a container's own
    --cpuset-cpus limit, or a SLURM job's own per-task core
    allocation."""
    try:
        available = len(os.sched_getaffinity(0))
    except AttributeError:
        available = os.cpu_count() or 1
    return max(1, min(6, available))
...
def write_rmclean_template(path: str) -> None:
    """A ready-to-edit rmclean_cubes cfg, generated alongside the
    pipeline cfg even though rmclean isn't chained into its stages= by
...
# stopping criteria are your call, not a safe default -- see that
# cfg's own header comment). To use: look at the rmsynth output first,
# then add 'rmclean' to the pipeline cfg's stages= and point its
# rmclean_cfg_template= at this file. ampfile/phafile/maskfile/outfile
# below are placeholders -- run_pipeline.sh overwrites them from the
# rmsynth stage's own output when both stages run together.
ampfile   = placeholder
phafile   = placeholder
...
def generate_pipeline_cfg(outdir: str, run_name: str, records: list,
    ...
    rmclean_template_path = os.path.join(outdir, f"{run_name}_rmclean.cfg")
    ...
    if chain_rmclean:
        ...
        header = (
            ...
            f"# a choice for you to make (see docs/user/EXAMPLES.md), not a safe\n"
            f"# default. A starter cfg is already generated at:\n"
            f"#   {rmclean_template_path}\n"
    ...
    if omp_threads < 6:
        logger.info(f"  This host has only {omp_threads} CPU(s) available -- "
              f"rmsynth_omp_threads set to {omp_threads} in the generated "
              f"cfg instead of the usual default of 6.")
    ...
    with open(pipeline_cfg_path, "w") as f:
        f.write(
            ...
            f"rmsynth_omp_threads = {omp_threads}\n"
            + (f"\n{rmclean_cfg_line}" if rmclean_cfg_line else "")
        )
    return pipeline_cfg_path


def unblock_stuck_keyring() -> None:
    """Probes the OS keyring with a bounded timeout before astroquery's
    own login touches it. A Secret Service-backed keyring can hang
    indefinitely instead of raising when a D-Bus session exists but no
    provider is registered on it (confirmed on a headless Pi: no
    exception, no timeout of its own -- just silence after "Target
    position: ...", indistinguishable from a dead network call).
    astroquery's QueryWithLogin._get_password() already wraps
    keyring.get_password() in try/except keyring.errors.KeyringError,
    so a raised error was never the gap here -- only a call that never
    returns defeats that.

    Uses a daemon threading.Thread, not ThreadPoolExecutor: confirmed
    directly against CPython's own concurrent/futures/thread.py that a
    ThreadPoolExecutor worker is joined by an interpreter-exit atexit
    hook (_python_exit, registered process-wide the first time any
    ThreadPoolExecutor is used) regardless of calling shutdown(wait=
    False) on that specific executor -- wait=False only skips blocking
    inside the shutdown() call itself, not the later atexit join. A
    stuck probe would therefore still hang the script at exit, just
    later than at the probe site. A daemon thread carries no
    such join and is killed outright when the process exits.

    On timeout, swaps in keyring.backends.null.Keyring() so every
    later keyring call this run makes resolves instantly instead of
    hanging, and tells the user why a password prompt is coming
    instead of a silent cached read."""
    result: queue.Queue = queue.Queue(maxsize=1)

    def probe() -> None:
        try:
            result.put(keyring.get_password("casda_fetch_probe", "probe"))
        except Exception as exc:
            result.put(exc)
    ...
    try:
        ...
        logger.info(
            "Password manager isn't responding -- this system likely "
            "has no secret-service backend running (common on headless "
            "machines). Falling back to a plain password prompt for "
            "this run; it won't be remembered."
        )
        keyring.set_keyring(keyring.backends.null.Keyring())
        return
    if isinstance(outcome, Exception):
        # keyring itself raised quickly (e.g. no backend configured at
        # all) -- astroquery's own _get_password() already catches
        # keyring.errors.KeyringError and falls back to getpass, so
        # this probe has nothing further to do.
        pass
...
def main() -> None:
    args = parse_args()
    if args.abort:
        # Standalone mode: no outdir/logging/target needed, and no fresh
        # OPAL password either -- a job's own URL is itself what CASDA
        # checks to authorize cancelling it (confirmed directly,
        # 2026-09-21). Plain print(), not the logger: this isn't a fetch
        # run, there's no outdir to put a log file in.
        exit_code = 0
        for job_url in args.abort:
            job_url = job_url.rstrip("/")
            print(f"Aborting {job_url} ...")
            try:
                resp = requests.post(
                    f"{job_url}/phase", data={"phase": "ABORT"}, timeout=30)
                print(f"  -> HTTP {resp.status_code}")
                if resp.status_code >= 400:
                    # Most likely: the job was already in a terminal phase
                    # (COMPLETED/ABORTED/ERROR) -- confirmed directly
                    # (2026-09-21) that re-aborting an already-terminal job
                    # returns 400, not a silent success.
                    exit_code = 1
            except requests.RequestException as exc:
                print(f"  -> failed to send the cancel request: {exc}")
                exit_code = 1
        sys.exit(exit_code)
    # Absolutized once, here, so every path derived from it downstream
    # (obs_dir, the pipeline_staging/ symlinks, the generated cfgs' own
    # outdir=/match_input_path=/*_cfg_template= values) is absolute too --
    # confirmed: a relative --outdir breaks scripts/run_pipeline.sh's
    # OWN match-stage symlinking (ln -s "${f}" "${link}" with a relative
    # ${f} creates a symlink whose target resolves against the new
    # symlink's own directory, match_input_symlinks/, not any sensible
    # base), causing "cannot open FITS file" once match_cubes tries to
    # read through it -- independent of anything my own stage_symlink()
    # does (that one already used an absolute target).
    args.outdir = os.path.abspath(args.outdir)
    os.makedirs(args.outdir, exist_ok=True)
    log_path = setup_logging(args.outdir)
    # Casda's own module-level singleton (this is what Casda.query_region
    # below calls through) has its own requests.Session, separate from
    # the casda = Casda() instance created a few lines down -- both need
    # the hook, or the discovery query's own request would be missing
    # from the log.
    register_request_logging(Casda._session)
    ...
    unblock_stuck_keyring()
    logger.info("Authenticating with CASDA (checking for a cached password)...")
    casda = Casda()
    register_request_logging(casda._session)
    # casda.login()'s own return value is always None on this astroquery
    # version -- astroquery.query.LoginABCMeta's generated login() wrapper
    # calls the underlying _login() implementation but never returns its
    # result (confirmed directly against astroquery 0.4.11's source: the
    # wrapper is missing a `return`). casda.authenticated() reads the
    # same self._authenticated the underlying implementation sets, so
    # it's the reliable way to check.
    casda.login(username=args.username, store_password=True)
    if not casda.authenticated():
        logger.error(
            "Authentication with CASDA failed -- check your OPAL username "
            "and password (astroquery logged the reason above). Nothing "
            "was queried or downloaded."
        )
        sys.exit(1)

    logger.info(f"Querying CASDA within {args.radius} arcmin...")
    ...
    logger.info(f"{len(result)} product(s) found, {len(public)} publicly released.")
    if len(public) == 0:
        return
    ...
    # EVERY requested observation succeeds (see the all-succeeded
    ...
    # original name -- confirmed directly). Instead, each
    ...
    for obs, version in selection:
        ...
        if args.if_exists == "clean" and os.path.exists(obs_dir):
            logger.info(f"  Removing {obs_dir} (--if-exists=clean)")
        ...
        logger.info(f"  {len(rows)} file(s) selected (one per Stokes parameter found, "
              f".conv preferred over plain restored where both exist)")

        if len(rows) == 0:
            logger.info(f"  SKIPPED: Q and/or U lack a .conv. variant -- nothing "
        ...
        # Predicted from conv_by_letter (already known pre-fetch, from
        # the archive filename) rather than checked against anything
        # CASDA-side: the cutout service renames every download to its
        # own generic cutout-<job>-imagecube-<id>.fits, so there is no
        # CASDA-side name to compare against locally -- only the FINAL,
        # post-rename tag this same fetch loop would itself produce
        # (see the rename step below) is checkable. Presence only, not
        # correctness: a version tag isn't recoverable from the
        # downloaded file or its renamed name either (see
        # version_number_from_filename), so an existing file can't be
        # verified to be the SPECIFIC version this run would have
        # requested -- --if-exists=check (the default) exists precisely
        # so this isn't silently assumed either way.
        expected_files = {
            letter: (f"{letter}_conv.fits" if is_conv else f"{letter}.fits")
            for letter, is_conv in conv_by_letter.items()
        }
        existing_files = {
            letter: os.path.join(obs_dir, fname)
            for letter, fname in expected_files.items()
            if os.path.exists(os.path.join(obs_dir, fname))
        }
        all_already_present = (
            expected_files and len(existing_files) == len(expected_files))

        n_ok = 0
        n_total = len(rows)
        written: dict = {}
        if all_already_present and args.if_exists != "refetch":
            existing_names = sorted(os.path.basename(p) for p in existing_files.values())
            if args.if_exists == "reuse":
                logger.info(f"  Reusing existing output already in {obs_dir}: "
                      f"{existing_names}")
                written = dict(existing_files)
                n_ok = n_total
            else:
                logger.info(f"  Found existing output already in {obs_dir}: "
                      f"{existing_names}")
                logger.info(f"  Not fetched again -- pass --if-exists=refetch to "
                      f"re-download and overwrite, or --if-exists=reuse to "
                      f"skip the download and use these files as-is.")
                obs_results.append((obs.obs_id, "EXISTS (needs --if-exists)"))
                all_succeeded = False
                continue
        else:
            try:
                logger.info(f"  Requesting a cutout from CASDA -- this stages "
                      f"the data server-side and polls until the job "
                      f"completes before download can start, which has "
                      f"taken upwards of a minute per observation. "
                      f"astroquery logs the job URL and phase changes "
                      f"below as it polls.")
                urls = casda.cutout(rows, coordinates=coord, radius=cutout_radius * u.arcmin,
                                     verbose=True)
                # cutout() only returns once every job it created reaches
                # COMPLETED -- nothing left here for Ctrl+C to cancel until
                # the NEXT observation's own cutout() call creates a new one.
                _pending_job_urls.clear()
                ...
                for path in fits_paths:
                    ...
                    logger.info(f"  wrote {os.path.basename(new_fits)}")
                ...
                # Only _complete_job()'s own explicit terminal-status check
                # raises ValueError -- that means polling got a definitive
                # answer (the job reached a non-COMPLETED terminal phase,
                # nothing left to retrieve). Any OTHER exception (a
                # network/connection error, a timeout, ...) means we
                # never got a definitive answer at all -- the job itself
                # may still be alive and progressing on CASDA's servers
                # right now, unknown from here, and could complete with
                # data this script has no way to retrieve once its own
                # tracking of the job ends below. Confirmed directly
                # (2026-09-21): a requests.exceptions.ConnectionError
                # mid-poll left the underlying job still PENDING on
                # CASDA's own server, unaffected by the script's own
                # failure.
                still_unknown_job_urls = list(_pending_job_urls)
                _pending_job_urls.clear()
                logger.info(f"  FAILED: {type(exc).__name__}: {exc}")
                if isinstance(exc, ValueError):
                    logger.info(
                        f"  (the job itself reached a definite "
                        f"non-COMPLETED status -- nothing to retrieve "
                        f"for this observation. The rest of this run "
                        f"will still be attempted, but since this "
                        f"observation didn't fully succeed, no pipeline "
                        f"config will be generated at the end -- see "
                        f"the summary below)"
                    )
                else:
                    logger.info(
                        f"  This was a connection/network problem, not "
                        f"a definite answer from CASDA -- the staging "
                        f"job itself may still be running on CASDA's "
                        f"servers right now and could complete with "
                        f"data this script has no way to retrieve, "
                        f"since its own tracking of the job ends here."
                    )
                    if still_unknown_job_urls:
                        logger.info(f"  Job(s) of unknown status:")
                        for job_url in still_unknown_job_urls:
                            logger.info(f"    {job_url}")
                        logger.info(
                            f"  Check its status directly (paste the URL "
                            f"in a browser, or curl it); cancel it if "
                            f"you don't want the result: python3 "
                            f"{os.path.basename(__file__)} --abort "
                            f"<url above>; or just re-run this fetch, "
                            f"which creates a fresh job independent of "
                            f"this one."
                        )
                    logger.info(
                        f"  The rest of this run will still be "
                        f"attempted, but no pipeline config will be "
                        f"generated at the end -- see the summary below."
                    )
        if n_ok == n_total:
            status = "OK"
        elif n_ok == 0:
        ...
        if "Q" in written and "U" in written:
            ...
            if has_beams_table(written["Q"]) and has_beams_table(written["U"]):
                logger.info(f"  Q and U both already carry a per-channel BEAMS "
                      f"table -- convolve_cubes' beamfiles=auto will read "
                      f"it directly, no separate beamlog needed.")
            else:
                logger.info(f"  Q and/or U's .conv cube has no BEAMS table of "
                ...
                if beamfile is None:
                    ...
                    logger.info(f"  {obs.obs_id}'s .conv cube has no BEAMS table "
...
if __name__ == "__main__":
    try:
        ...
        # abort_pending_jobs() itself is a no-op (silent, immediate return)
        # if nothing is currently staged -- e.g. interrupted before any job
        # existed yet, or after the last one already completed. In that
        # case this is just an ordinary interrupt, no special handling: a
        # quiet exit(130), not a scary traceback.
        abort_pending_jobs()
        sys.exit(130)
