...
def generate_pipeline_cfg(outdir: str, run_name: str, records: list,
    ...
    if omp_threads < 6:
        print(f"  This host has only {omp_threads} CPU(s) available -- "
...
def main() -> None:
    ...
    print("Authenticating with CASDA (checking for a cached password)...")
    casda = Casda()
    ...
    # calls the real _login() implementation but never returns its result
    # (confirmed directly against astroquery 0.4.11's source: the wrapper
    # is missing a `return`). casda.authenticated() reads the same
    # self._authenticated the real implementation does set, so it's the
    ...
    for obs, version in selection:
        ...
        if all_already_present and args.if_exists != "refetch":
            ...
            if args.if_exists == "reuse":
                print(f"  Reusing existing output already in {obs_dir}: "
                      f"{existing_names}")
                written = dict(existing_files)
                n_ok = n_total
            else:
                print(f"  Found existing output already in {obs_dir}: "
                      f"{existing_names}")
                print(f"  Not fetched again -- pass --if-exists=refetch to "
            ...
            try:
                print(f"  Requesting a cutout from CASDA -- this stages "
