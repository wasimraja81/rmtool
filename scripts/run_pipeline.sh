...
# Found needed: a real multi-band run wanted match_cubes'
...
# path=/infileQ=/infileU= pointed at the first invocation's
...
#
# --- match_cubes output location (a constraint, not a gap here) ---
...
# inherit it by default; the rmsynth stage below overrides
# OMP_NUM_THREADS per-run (rmsynth_omp_threads=) and restores
# DEFAULT_OMP_NUM_THREADS afterward, so a per-stage override there
# never leaks into the rmclean stage that may follow it in the same
# invocation.
DEFAULT_OMP_NUM_THREADS=6
export OMP_NUM_THREADS="${DEFAULT_OMP_NUM_THREADS}"
...
if [[ "${DO_MATCH}" -eq 1 ]]; then
  ...
  # Resolve to absolute now, before any symlink below is built from it --
  # match_cubes' own output lands ALONGSIDE its input via a symlink (see
  # MATCH_SYMLINK_DIR below), and a symlink created with a relative
  # target resolves against the symlink's OWN directory, not this
  # script's cwd -- a relative match_input_path here would silently
  # produce a symlink pointing nowhere real (confirmed: cfg/*.cfg's own
  # match_input_path= is written relative to $RMTOOL_HOME per this
  # project's own convention, not absolute).
  [[ "${MATCH_INPUT_PATH}" != /* ]] && MATCH_INPUT_PATH="${ROOT_DIR}/${MATCH_INPUT_PATH}"
...
if [[ "${DO_RMSYNTH}" -eq 1 ]]; then
  ...
  [[ -z "${RMSYNTH_THREADS}" ]] && RMSYNTH_THREADS="${DEFAULT_OMP_NUM_THREADS}"
  if [[ "${RMSYNTH_BACKEND}" != "auto" && "${RMSYNTH_BACKEND}" != "cpu" && "${RMSYNTH_BACKEND}" != "gpu" ]]; then
    die "rmsynth_backend must be one of auto|cpu|gpu, got: ${RMSYNTH_BACKEND}"
  fi
  ...
  # Backend selection: cpu/gpu pick their own binary directly; auto reads
  # use_gpu/use_gpus from the rmsynth cfg itself (both accepted aliases,
  # confirmed in src/rm_synthesis_mod.f90's own parser: case ('use_gpu',
  # 'use_gpus')). Inlined here (rather than delegating to a separate
  # runner script) to match how match_cubes/rmclean_cubes are already
  # invoked directly above/below in this same file, with no wrapper.
  if [[ "${RMSYNTH_BACKEND}" == "cpu" ]]; then
    RMSYNTH_USE_GPU="n"
  elif [[ "${RMSYNTH_BACKEND}" == "gpu" ]]; then
    RMSYNTH_USE_GPU="y"
  else
    USE_GPU_CFG="$(cfg_get "${RMSYNTH_CFG}" use_gpu)"
    [[ -z "${USE_GPU_CFG}" ]] && USE_GPU_CFG="$(cfg_get "${RMSYNTH_CFG}" use_gpus)"
    if [[ -n "${USE_GPU_CFG}" ]] && flag_is_true "${USE_GPU_CFG}"; then
      RMSYNTH_USE_GPU="y"
    else
      RMSYNTH_USE_GPU="n"
    fi
  ...
  if [[ "${RMSYNTH_USE_GPU}" == "y" ]]; then
    RMSYNTH_EXE="${ROOT_DIR}/bin/rm_synthesis_release_gpu_offload_hostomp"
  else
    RMSYNTH_EXE="${ROOT_DIR}/bin/rm_synthesis_release_cpu_omp"
  fi
  if [[ ! -x "${RMSYNTH_EXE}" ]]; then
    die "rm_synthesis not built -- run: make OMP=1 GPU=$([[ "${RMSYNTH_USE_GPU}" == y ]] && echo 1 || echo 0) (${RMSYNTH_EXE})"
  fi

  log "Running: ${RMSYNTH_EXE} ${RMSYNTH_CFG} (backend=${RMSYNTH_BACKEND}, effective use_gpu=${RMSYNTH_USE_GPU}, OMP_NUM_THREADS=${RMSYNTH_THREADS})"
  export OMP_NUM_THREADS="${RMSYNTH_THREADS}"
  if [[ "${RMSYNTH_USE_GPU}" == "y" ]]; then
  ...
  run_logged "${PROVDIR}/rmsynth.${RUN_TAG}.stdout.log" \
    "${RMSYNTH_EXE}" "${RMSYNTH_CFG}"
  # Restore the default so a per-stage rmsynth_omp_threads= override
  # doesn't leak into the rmclean stage that may follow in the same
  # invocation.
  export OMP_NUM_THREADS="${DEFAULT_OMP_NUM_THREADS}"
