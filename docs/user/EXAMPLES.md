...
synthetic test fixtures, and the output shown is what it
produces. A few scenarios need data this repo doesn't ship (e.g. two
bands with different angular resolution) — those are noted
where they come up.

Run every command below from `$RMTOOL_HOME` -- the directory you
cloned this repository into (see
...
Multi-band data comes in four combinations: your bands might
...
resolutions (a very common case — different frequencies naturally
...
this is the normal case for any cube with a per-channel `BEAMS`
...
every input can be deconvolved from — you don't need to work
...
- **`dry_run=y`** exists only on `rm_synthesis`: it reads headers, runs
  ...
  `tile_ra`/`tile_dec` you can copy back into your cfg) and
