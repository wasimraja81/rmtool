...
Run everything from `$RMTOOL_HOME` -- the directory you cloned this
repository into (see [QUICKSTART.md](../../QUICKSTART.md#where-things-go)
for the full convention; setting it as an actual env var is optional).
...
[QUICKSTART.md](../../QUICKSTART.md#diagnosing-build-failures) if
`make` fails — it covers prerequisites and points at
[BUILD.md](../../BUILD.md#troubleshooting) for every specific failure
mode.
...
or copy `cfg/rmsynth-e2e-smalltest.cfg` as a starting point -- its
`path=` is already `tests/data/`, correct as long as you're running
from `$RMTOOL_HOME` per the note above:
...
and stops immediately at `abs_flux_floor`). Real data with a
...
- [QUICKSTART.md](../../QUICKSTART.md) — the same run above, end to
  end in two commands; fetching your own data; validating your build.
