...
(`docs/dev/IO_PARALLEL_OPTIMISATION_PLAN.md`): Objective / Scope / Change
...
### T3b — I/O Orchestration Cleanup (`io_overlap` ping-pong + write-job dispatch, elevated caution) — DONE
...
- **Why elevated caution:** two real production SIGSEGVs happened one function-call away from this exact code (CFITSIO handle aliasing, write-vs-write race — both documented in `docs/user/ARCHITECTURE.md`'s postmortems). Both fixes are timing-dependent invariants (join-before-reuse, join-before-dispatch) that are trivially easy to reorder while "cleaning up," with no compiler error and — per both postmortems explicitly — no small/fast-test failure to catch it.
...
- **Evidence (2026-07-19):** New `populate_write_job` (`rm_synthesis_mod.f90`, next to `do_tile_write`) holds the ~47-line `write_job(cur_slot)%field = local` assembly, moved verbatim (only `var` → `argument`); called from the tile loop exactly where the assignment block used to be. Checked `git diff` **before** running anything, per the rollback criterion: only the assembly block and two comments changed, confirmed by inspection that every synchronisation statement (`write_pending` checks, `tile_write_join`, `tile_write_dispatch_async`, `do_tile_write`, and their relative order) is untouched. Also landed the stale-comment fix at the `if(io_overlap)` block ("`io_write_threads_eff is hard-clamped to 1`" → updated to match the "Handle safety" explanation already in `docs/user/ARCHITECTURE.md`'s Synchronisation section, including why the rule is now kept defensively rather than because of a hard clamp that no longer exists). Ping-pong block (`cur_slot`/`write_pending`/pointer re-association) left untouched per the Change Set — only the write-job assembly was in scope. Clean 4-variant rebuild: 0 errors, 0 new warnings. `tests/run_tests.sh`: 28/28 pass, including §13 (`require_no_overlapping_tile_writes`) and §14 (`io_write_threads` bit-identical) — the primary signal for this ticket. Full bit-identical sweep of all 140 T0-archived FITS outputs: same known 6-file NaN-artifact class as T1/T2/T3a, not a regression. On the optional "larger/slower re-run" (real 4501×4501 `io_overlap=y` data was available locally via that dataset's own full-image rmsynth cfg): asked the user first given the ~1hr real-data run cost; decided to skip it, since the diff provably never touches the timing-dependent code the postmortems warn about (confirmed by the `git diff` check above) — the in-suite §13/§14 result is the relevant signal for a change of this shape, unlike a ticket that touched the synchronisation itself.

---

### T5 — Deep Config Threading (cfg% everywhere) — final, lowest-priority ticket — DONE
...
- **Change Set:** Split into sub-tickets by variable cluster, each independently gated by the full suite — do not attempt as one large diff:
  - T5a: `cfg%tile_ra`/`cfg%tile_dec` and related planner inputs (highest usage count: `tile_ra` alone appears at 69 sites, `tile_dec` at 64). **DONE (2026-07-19):** removed the local `tile_ra`/`tile_dec` declaration and their T1 unpack lines entirely; T2's `plan_tile` call site now reads `cfg%tile_ra`/`cfg%tile_dec` directly as `plan%tile_ra_in`/`plan%tile_dec_in` and writes the auto-tiler's resolved result straight back into `cfg%tile_ra`/`cfg%tile_dec` (the coupling T2's objective already flagged: cfg-parsed and auto-tuner-overwritten need one source of truth, now `cfg%` itself rather than a loose local mirror). All other ~47 remaining bare-identifier sites (RAM/VRAM allocations, tile-loop bounds, dry-run `tile_autotune.cfg`/`runtime_estimate.txt` writer, `write_runtime_estimate` call) renamed via a quote-aware word-boundary substitution script (same technique as T1's cfg rename), restricted to the tile-loop/planner line range so the two unrelated prose comments mentioning `tile_ra*tile_dec` generically elsewhere in the file were left untouched, and the string literals `"tile_ra="`/`"tile_dec="`/`"...smaller tile_dec."` (real KEY=VALUE text written to the autotune cfg / a diagnostic message) were correctly skipped by the quote-aware splitter. Clean 4-variant rebuild: 0 errors, 0 new warnings. `tests/run_tests.sh`: 28/28 pass. Full bit-identical sweep of all 140 T0-archived FITS outputs: same known 6-file NaN-artifact class as every prior ticket, not a regression. `tile_autotune.cfg`/`runtime_estimate.txt` diffed byte-identical against the copies saved right after T2 (unaffected by T3a/T3b, so a valid comparison point).
  - T5b: `cfg%io_*` (io_overlap: 22 sites, io_write_threads: 16, io_read_threads similar). **DONE (2026-07-19):** removed the local `io_overlap`/`io_read_threads`/`io_write_threads` declarations and their T1 unpack lines (the `_eff` clamped locals stay -- those are genuinely computed, not cfg-mirrors). T2's `plan_tile` call site now reads `cfg%io_overlap` directly. All other bare-identifier sites renamed via the same quote-and-comment-aware substitution script as T5a, this time whole-file (not line-range-restricted, since these three keys' real code sites are scattered across the file rather than confined to one block) with an added negative-lookbehind guard so already-prefixed `cfg%io_overlap`/`plan%io_overlap` occurrences aren't double-prefixed. Comment-only lines and log-message string literals (`'io_overlap=y: async tile-write pthread active'`, `"io_read_threads="`, etc.) were correctly left untouched. `git diff` inspected line-by-line before testing, per the same caution as T3b since this touches the `io_overlap`-gated concurrency branches directly (T3b's own scope, revisited): confirmed pure identifier substitution only, no statement added/removed/reordered. Clean 4-variant rebuild: 0 errors, 0 new warnings. `tests/run_tests.sh`: 28/28 pass, including §13/§14 which directly exercise this code. Full bit-identical sweep of all 140 T0-archived FITS outputs: same known 6-file NaN-artifact class as every prior ticket.
  ...
  - **T5c progress (2026-07-19):** batch 1 done -- `mem_frac_ram`/`mem_frac_vram`/`gpu_vram_mib`/`tile_auto`/`dry_run` (the rest of the tile/memory-planning cluster T5a started, plus `dry_run`) threaded to `cfg%`. Same guarded quote-and-comment-aware substitution as T5a/T5b (lookbehind excludes already-`cfg%`/`plan%`-prefixed occurrences, since `tile_plan_t` has its own identically-named `mem_frac_ram`/`mem_frac_vram`/`gpu_vram_mib`/`tile_auto` fields that must stay untouched). Clean 4-variant rebuild, 28/28 tests, full bit-identical FITS sweep (same known artifact class), `tile_autotune.cfg`/`runtime_estimate.txt` re-verified byte-identical against the T2 baseline copies.
  - **T5c progress, batch 2 (2026-07-19):** Q/U bias-correction cluster done -- `rem_mean`/`remove_qu_bias`/`resiQ`/`slopeQ`/`resiU`/`slopeU`/`path_I`/`infileI` threaded to `cfg%`. Two wrinkles handled: (1) `tile_plan_t` also has its own `rem_mean` field (`plan%rem_mean`), guarded the same way as the batch-1 collisions; (2) the local `remove_qu_bias` was actually declared/assigned as `remove_QU_bias` (mixed case) but referenced lowercase at its two usage sites -- Fortran identifiers are case-insensitive so this was always one variable, handled with a case-insensitive substitution pass so both spellings map to `cfg%remove_qu_bias`. Also confirmed `slopeQ`/`slopeU` have zero usage sites beyond the old unpack line (dead cfg values, pre-existing, not something this ticket introduces or should "fix" beyond removing their now-redundant local mirror) -- nothing to substitute for those two once the declaration/unpack lines were removed. Clean 4-variant rebuild, 28/28 tests, full bit-identical FITS sweep (same known artifact class).
  - **T5c progress, batch 3 (2026-07-19):** RM sampling cluster done -- `ofac`/`fac`/`beg_rm`/`end_rm`/`nrm_out_par`/`use_auto_rm_range` threaded to `cfg%`. `nrm_out_par` is mutated in place when `use_auto_rm_range=1` (`nrm_out_par = nz_out`) -- same "cfg% is the single live value, auto-computed overwrites write back into it" pattern established for `tile_ra`/`tile_dec` in T5a, now `cfg%nrm_out_par = nz_out`. `nrm_out` itself is a derived local (not a cfg field) and correctly left alone throughout. Clean 4-variant rebuild, 28/28 tests, full bit-identical FITS sweep (same known artifact class).
  - **T5c progress, batch 4 (2026-07-19):** output-mode + masking cluster done -- `output_mode`/`ap_angle_mode`/`mask_cube_file`/`mask_input_cube_file`/`mask_trust_mode`/`write_mask_output`/`write_nvalid_output` threaded to `cfg%`. `mask_input_cube_file`'s self-referencing path-prepend assignment (`mask_input_cube_file(1:) = path(...)//mask_input_cube_file(...)`) converts the same safe way `infileI`'s did in batch 2. Clean 4-variant rebuild, 28/28 tests, full bit-identical FITS sweep (same known artifact class).
  - **T5c progress, batch 5 (2026-07-19):** `cubestat`/`use_gpu` threaded to `cfg%`. `tile_plan_t` has its own `cubestat` field (`plan%cubestat`), guarded the same way as prior collisions; `use_gpu_actual` (a distinct, GPU-availability-resolved local) correctly left untouched throughout by the word-boundary regex. Clean 4-variant rebuild, 28/28 tests, full bit-identical FITS sweep (same known artifact class).
  - **T5c progress, batch 6 (2026-07-19):** logging/timing cluster done -- `log_level`/`timing_enabled`/`timing_tile_enabled`/`timing_io_enabled`/`log_output_file`/`timing_csv_file` threaded to `cfg%`. This was the tail end of the T1 unpack bridge block for these fields (nothing else followed it in the block). Clean 4-variant rebuild, 28/28 tests (including §12 timing report/CSV validation), full bit-identical FITS sweep (same known artifact class).
  - **T5c progress, batch 7 -- final (2026-07-19):** the path/badchan/subim clusters done -- `path`/`infileQ`/`infileU`/`outfile`/`remove_badchan`/`badchan_file`/`subim`/`subim_parfile`/`subim_ra_blc`/`subim_ra_trc`/`subim_ra_inc`/`subim_dec_blc`/`subim_dec_trc`/`subim_dec_inc`/`subim_chan_blc`/`subim_chan_trc`/`subim_chan_inc` threaded to `cfg%`. One name mismatch handled: the local mirror was called `global_badchan_file` but the cfg field is `badchan_file` -- mapped explicitly (`global_badchan_file` → `cfg%badchan_file`), not a mechanical same-name substitution. `remove_badchan`'s fallback-on-open-failure mutation (`remove_badchan = .false.`) now writes back into `cfg%remove_badchan`, same pattern as `tile_ra`/`nrm_out_par`. This was the last cluster in the T1 unpack bridge block, so the block (and its now-obsolete "temporary bridge, goes away once T5 lands" comment) was deleted entirely and replaced with a two-line note that T5 completed the threading. Clean 4-variant rebuild -- with `implicit none` in force, this is a hard proof no bare reference to any removed declaration survived (an implicit-typing bug would have been a compile error, not silent). 28/28 tests, full bit-identical FITS sweep (same known artifact class), `tile_autotune.cfg`/`runtime_estimate.txt` re-verified byte-identical against the T2 baseline.

**T5 is now fully DONE** -- all ~56 `rmsynth_config_t` fields are read as `cfg%field` directly at every use site in `rm_synthesis.f90`; the T1 shallow-encapsulation unpack bridge no longer exists. This closes the encapsulation refactor plan's full ticket list (T0 → T1 → T2 → T3a → T3b → T5a/b/c), each independently verified via clean 4-variant rebuild + full 28-test suite + bit-identical FITS sweep against the frozen T0 baseline.
...
4. Track progress by updating each ticket's status (OPEN → IN_PROGRESS → DONE, with evidence) directly in this plan file or a promoted copy under `planning/`, matching the convention already used in `docs/dev/IO_PARALLEL_OPTIMISATION_PLAN.md`.
