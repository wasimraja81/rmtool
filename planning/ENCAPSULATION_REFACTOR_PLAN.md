...
- **Change Set:** Split into sub-tickets by variable cluster, each independently gated by the full suite — do not attempt as one large diff:
  ...
  - Remaining T5c clusters not yet done: `log_level`/`timing_enabled`/`timing_tile_enabled`/`timing_io_enabled`/`log_output_file`/`timing_csv_file`, and the path/badchan/subim clusters (`path`/`infileQ`/`infileU`/`outfile`, `remove_badchan`/`badchan_file`, `subim`/`subim_parfile`/`subim_ra_*`/`subim_dec_*`/`subim_chan_*`).
