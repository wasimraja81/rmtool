...
### T5 — Deep Config Threading (cfg% everywhere) — final, lowest-priority ticket — IN PROGRESS
...
- **Change Set:** Split into sub-tickets by variable cluster, each independently gated by the full suite — do not attempt as one large diff:
  ...
  - Remaining T5c clusters not yet done: the path/badchan/subim clusters (`path`/`infileQ`/`infileU`/`outfile`, `remove_badchan`/`badchan_file`, `subim`/`subim_parfile`/`subim_ra_*`/`subim_dec_*`/`subim_chan_*`) -- these are the last fields, at which point the T1 unpack bridge block becomes fully empty and can be deleted entirely, closing T5 in full.
