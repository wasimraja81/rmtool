...
#
# Dependency checks do a trivial link against each library
# (exactly what the build does) rather than searching
...
# previous session, or copied from docs), it must agree with
# where this script is -- a stale value (moved/renamed clone,
...
if [[ -n "${RMTOOL_HOME:-}" ]]; then
  ...
  if [[ "${RMTOOL_HOME_RESOLVED}" != "${ROOT_DIR}" ]]; then
    echo "[build] ERROR: \$RMTOOL_HOME ('${RMTOOL_HOME_RESOLVED}') does not match where this script is ('${ROOT_DIR}')." >&2
    echo "[build] Either unset \$RMTOOL_HOME, or fix it to point here: export RMTOOL_HOME=\"${ROOT_DIR}\"" >&2
    exit 1
  fi
fi
...
# Link test against gfortran -- the same mechanism the
# build uses (Makefile's CFITSIO_LIB/AST_LIBS/FFTW_LIBS are plain -l
# flags, no -L path, so "can gfortran link this" is the check).
check_lib() {
...
log "Setting up the Python venv (tests/run_tests.sh, scripts/casda_fetch.py, plotting tools) -- best effort, not required for the binaries above"
VENV_SET_UP=0
if ! command -v python3 >/dev/null 2>&1; then
  echo "  [MISSING] python3 not found -- skipping venv setup (compiled binaries above are unaffected)."
  echo "      Debian/Ubuntu: sudo apt-get install python3 python3-venv python3-pip"
  echo "      macOS:         python3 ships with Xcode Command Line Tools (xcode-select --install)"
  echo "      Then re-run scripts/setup_venv.sh yourself before using tests/run_tests.sh or scripts/casda_fetch.py."
elif bash scripts/setup_venv.sh; then
  VENV_SET_UP=1
else
  echo "  Python venv not set up -- see above for the error (often no network access, or python3-venv/python3-pip not installed alongside python3). Compiled binaries are unaffected; re-run scripts/setup_venv.sh yourself once fixed."
fi
...
if [[ "${VENV_SET_UP}" -ne 1 ]]; then
  echo "  scripts/setup_venv.sh   # needed for tests/run_tests.sh, scripts/casda_fetch.py, and plotting tools -- couldn't set this up automatically above, see the message there"
fi
