...
```bash
git clone https://github.com/wasimraja81/rmtool.git ~/rmtool
cd ~/rmtool
```

`~/rmtool` is just a suggested default -- clone anywhere you like.
Run everything below **from inside that clone** -- these docs call
that directory `$RMTOOL_HOME`. Setting it as an actual env var is
optional (`export RMTOOL_HOME=/path/to/your/clone`, once, if you want
to copy-paste commands verbatim); nothing here requires it.

- `./build.sh` and every `make` command are run from `$RMTOOL_HOME`
  itself -- `cd "$RMTOOL_HOME"` first if you're not already there.
...
- The examples below run tools from `$RMTOOL_HOME` using relative
  paths (`bin/...`, `cfg/...`). A couple (marked where they appear)
  `cd` into `$RMTOOL_HOME/scratch/` first, specifically to keep
  generated output cubes out of the repo root -- their own relative
  paths (`../bin/...`) are correct from there, not from
  `$RMTOOL_HOME` directly.

---

## 1. Build variants and binary capabilities

Four independent binaries can be produced; each lives under
`$RMTOOL_HOME/bin/` and is also copied to `$RMTOOL_HOME/bin/rm_synthesis`
(last build wins the copy).
...
Run all commands from `$RMTOOL_HOME` (see "Where things go" above).
...
Two paths, depending on your situation: no Q/U data yet and want the
full fetch-and-run pipeline → §4a. Already have a matched Q/U cube
pair and just want to run `rm_synthesis` directly → §4b.
...
and `astroquery`, which isn't part of the core build's own
dependencies -- run `scripts/setup_venv.sh` once to create
`~/venv/rmtool` with this and every other Python dependency this
project uses (system Python is never touched), then activate it for
this shell session (`source ~/venv/rmtool/bin/activate`) so the plain
`python3` commands below resolve to it. The first run prompts for your
password once and remembers it via your OS keyring; every later run is
silent.
...
`outfile` (and `path`) are relative to wherever you invoke the binary
from -- `mkdir -p "$RMTOOL_HOME/scratch" && cd "$RMTOOL_HOME/scratch"`
first to keep generated cubes out of the repo root, or use absolute
paths instead.
...
mkdir -p "$RMTOOL_HOME/scratch" && cd "$RMTOOL_HOME/scratch"
...
mkdir -p "$RMTOOL_HOME/scratch" && cd "$RMTOOL_HOME/scratch"
OMP_TARGET_OFFLOAD=MANDATORY OMP_DEFAULT_DEVICE=0 \
...
```bash
# Edit cfg to set dry_run=y, then (from scratch/):
OMP_NUM_THREADS=1 OMP_PROC_BIND=close OMP_PLACES=cores \
    ../bin/rm_synthesis_release_cpu_omp ../cfg/rmsynth.cfg
cat tile_autotune.cfg          # copy tile_ra / tile_dec back into cfg
cat runtime_estimate.txt       # wall-time estimate
```
...
```bash
# Temporarily set dry_run=y in cfg, then (from $RMTOOL_HOME):
mkdir -p "$RMTOOL_HOME/scratch" && cd "$RMTOOL_HOME/scratch"
OMP_NUM_THREADS=1 OMP_PROC_BIND=close OMP_PLACES=cores \
    ../bin/rm_synthesis_release_cpu_omp ../cfg/rmsynth.cfg
cat tile_autotune.cfg          # copy tile_ra / tile_dec back into cfg
...
| Python 3 + this project's Python dependencies (astropy, astroquery, matplotlib, numpy, pyvo, requests) | `scripts/setup_venv.sh` -- creates `~/venv/rmtool` and installs everything from `requirements.txt` via that venv's own pip (system Python untouched); see [§4a](#4a-getting-a-test-object) for activating it |
| ffmpeg (diagnostic animation only, not needed to build or run any Fortran tool, or any other Python script here) | via your package manager -- used only by `scripts/animate_fits_cube.py` |
