program test_table_oversample_floor
   ...
   ! max(l_sq)=0.140434 (>= span, no Gate-0 guarantee), but the
   ...
   ! [99,100] -- a ~30 MHz low-frequency band, not an unphysical
   ! number) that breaks even table_oversample=20 at zero mode. The
   ! floor must raise it, and the raised value must satisfy
