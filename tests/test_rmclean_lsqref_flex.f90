program test_rmclean_lsqref_flex
   ...
   !! scenario at two different compute references -- 0 (this
   ...
   !! development to be the fix for a chi0-precision gap at
   ...
   !! different) grid costs -- the answer to "can we keep the
   ...
   ! decouples chi0 precision from dRM (root-caused empirically:
...
contains
   ...
   &lsq_ref_report, chi0_tol, all_pass_io)
      ...
      ! (grid-quantised, up to dRM/2 biased) peak location was the
      ! bug behind the earlier ~0.22 rad discrepancy at a nonzero
      ! lsq_ref_compute -- not insufficient interpolation, a
      ...
      ! case (NOT derived from derotate_to_lsq_ref itself, to
