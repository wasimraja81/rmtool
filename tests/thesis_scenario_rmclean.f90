program thesis_scenario_rmclean
   ...
   !! is a poor noise-floor proxy for a Faraday-thick (broad,
   ...
   !! from the very first iteration -- confirmed an early-termination
   ...
   ! width, Jy) -- what a "total flux of an extended component"
   ...
   ! spread around a mean (~0.2) -- a Nyquist-type sampling
...
contains
   ...
   subroutine dirty_spectrum(l_sq, nchan, q, u, rm, nrm_l, dirty_re, dirty_im)
      ...
      !! throughout this file) -- a correctness requirement, not a
   ...
   &point_peak, point_rm_found, band_offset_in, band_nz_in, n_bands_in)
      ...
      ! comment on this output) -- a precision improvement over the
      ...
      ! flux, obviously unphysical) before being reported as a
   ...
   subroutine run_band_case(label, slug, nu_c, dnu, nchan, rm, nrm_l, outdir, all_pass_io)
      ...
      if (index(label, 'P-alone') > 0) then
         ...
         ! number (recovered RM is within ~0.1); 15% is "nearly
         ! equal power" (recovered amplitude is within ~2.5%).
   ...
   subroutine run_combined_case(rm, nrm_l, outdir, all_pass_io)
      ...
      ! evidence of the thesis's own claim). Two generous,
      ! qualitative thresholds: combining bands must recover CLEARLY more
      ! than P-alone does (5x is a generous multiplier given the
      ! ratio here is ~27x, comfortably robust against run-to-run detail
      ! changes), and P+L's own recovered fraction must be a
      ...
      ! not a target tuned to the ~40% observed.
