program test_gaussft_padding
   ...
   !! new nx_pad,ny_pad arguments) -- added after a ~46GB production
   ...
   !!    tests never exercised padding -- confirming why this
   ...
   !!    so the padding path activates), must produce EXACTLY
...
contains
   ...
   subroutine check_padding_equivalence(ok)
      ...
      ! Native (source) PSF, and a larger target PSF --
      ...
      ! needed (nx=ny=nx_pad=ny_pad=45), so this is an
      ...
      ! regression. 1.0e-6 stays comfortably above the observed
