...
! beam is added to the same pool before finding one common beam,
...
! target_bpa), which skips auto-derivation -- gaussft_mod itself
...
program convolve_cubes
   ...
   ! large images/generous mem_frac_ram combinations; ordinary
   ...
   ! at-a-time design rationale. block_write_job_t is self-
   ...
   ! needing an atomic: the background thread has already exited).
   ...
   ! (empty entry = none for that infile). Per-band, unlike the
   ...
   ! write I/O -- a real disk, 150MB/s measured -- as serial with
...
contains
   ...
   function strip_fits_ext(filename) result(base)
      ...
      !! stokesQ.fits_CONV.FITS" this used to produce by
   ...
   &nwriters, status)
      ...
      ! budgeted whether or not io_overlap is on, so turning it
      ...
      ! un-budgeted by the block buffer above.
      ...
      ! parallel do across planes), so this is the compute
      ...
      ! it. This still captures the win: write(N) overlaps
      ! read+compute(N+1), which is where the dead time was
      ! (measured directly: ~44s/block write, serial with compute,
      ...
      ! threads piggyback on cores idle at write-dispatch time
      ...
      ! comment for why) -- convolve_cubes always tracks a
   ...
   &tgt_bpa, status)
      ...
      !! the common target beam every good plane was convolved
      ...
      !! exactly which channels were convolved, rather than a
   ...
   subroutine copy_generic_header_convolve(src_unit, dst_unit, status)
      ...
      !! once every good channel has been convolved to one
      !! common target beam).
