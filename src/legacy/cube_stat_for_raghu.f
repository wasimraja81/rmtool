...
chelp-
      ...
      real*4    data_arr(maxchan)
      real*4 specRM(max_dec*maxchan) 
...
!       status  i  output error status (0 = ok)
     ...
     -                    nullval,specRM,anyflg,status)
         ...
         do iy = ypix_beg,ypix_end,incs(2)
            ...
            cnt1 = cnt1 + 1
            do i = 1,nz_use
               data_arr(i) = real(specRM(iypix_now + (i-1)*ny_use))
            enddo
            !----------------------------------------------------
            ngood_chan = 0
            ...
            !     write(*,*)"doing ",cnt1," out of",nx_use*ny_use 
            !endif
         ...
         !write(*,*)"group: ",group 
         !write(*,*)"naxes_out: ",naxes_out 
         !write(*,*)"fpixel: ",fpixels 
         !write(*,*)"lpixel: ",lpixels 
         call ftpsse(41,group,3,naxes_out,fpixels,lpixels,
     -                  maxloc_arr,status)
         !write(*,*)ix,"  -- status: ",status
         !stop
