...
!      write(*,*)" "
     ...
     -                        infile_X(1:nchar(infile_X))
      ...
      do k = 1,nused
         ...
         Q_rm_arr(k) = Q_arr(k) !LP_arr(k) * cos(PA_arr(k)) 
         U_rm_arr(k) = U_arr(k) !LP_arr(k) * sin(PA_arr(k)) 
         LP_arr(k) = sqrt(Q_rm_arr(k)**2 +  U_rm_arr(k)**2) 
         PA_arr(k) = 0.5*atan2(U_rm_arr(k), Q_rm_arr(k)) 
