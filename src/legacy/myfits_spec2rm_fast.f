...
!       status  i  output error status (0 = ok)
      ...
      do ix = xpix_beg,xpix_end,incs(1)
         do iy = ypix_beg,ypix_end,incs(2)
            ...
            do i = zpix_end,zpix_beg,-incs(3)
               if(flag_arr(i).eq.1)then
                       ngood_chan = ngood_chan + 1
                       Q_now(ngood_chan) = data_cubeQ(ix,iy,i)
                       U_now(ngood_chan) = data_cubeU(ix,iy,i)
               endif
            enddo
            !----------------------------------------------------
     ...
     -                           cos_arr,sin_arr, maxnt, maxchan)
            ...
            if(line_cut)then
                    ...
                    call ftpsse(41,group,naxis,naxes,fpixels,lpixels,
     -                             p_ex,status)
                    call ftpsse(42,group,naxis,naxes,fpixels,lpixels,
     -                             phi_ex,status)
            endif
