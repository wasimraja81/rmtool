chelp+
! This code displays the "help" section of a given program
! provided the source code for the program contains such a
! section.
! This code differs from the "manpage" subroutine in that 
! it can be used externally, to get "help" on any program
! that may have a help section, without actually executing 
! that program. 
! 
! All programs written by "yours truly" after Nov, 2009(and 
! before some future time when probably more sophisticated 
! ways of displaying the documentation will dawn upon him) 
! should contain a help section in the format expected by 
! this program! 
! 
!            --wasim raja, rri, 24 Nov, 2009
chelp-      
      ...
      if(iargc().lt.1)then
          write(*,*)" Usage: get_help <program-name.f (with full path)>"
          write(*,*)"        to get help on 'program-name.f' "
          write(*,*)"    OR: get_help -help"
          write(*,*)"        to get help on the current 'help' code "
          stop
      else
              call getarg(1,prg_name)
              if(index(prg_name,'-help').gt.0)then
                      prg_name = '/usr/lib/subroutine_lib/get_help.f'
              endif
      endif
