chelp+
      !-------------------------------------------------------------
      ! This code converts the FITS Q and the U spectral cubes or a 
      ! subset of it into ASCII format as would be recognised by codes 
      ! developed for pulsar tomography. 
      !                                    -- wr, 25 Oct, 2011
      !-------------------------------------------------------------
chelp-
     ...
     -          specQ(max_dec*maxchan), specU(max_dec*maxchan) 
      ...
      integer*4 rwmode
      character*272 infileQ, infileU, message
...
102   continue
      ...
      outfile_ascii(1:) = outfile(1:nchar(outfile))//'.QU.ASCII'
...
!       status  i  output error status (0=OK)
      ...
      write(39,*)nx_out,ny_out,2,nz_out,"! nra,ndec,nstk,nchan"
      atmp = 0.5*(zn + z1)/1.0e6
      btmp = (zn - z1)/1.0e6
      write(39,*)atmp,btmp,1.0,0.0,"! fcen_MHz,BW_MHz,P0,DM"
      write(39,*)"dummy ! src/filename"
      write(39,*)"GMRT ","! sitename"
      write(39,*)2011,10,26,00,00,00,"! date: yyyy,MM,DD,hh,mm,ss"
      write(39,'(a)')"00 00 00 ! ra_hh, ra_mm, ra_ss " 
      write(39,'(a)')"00 00 00 ! dec_dd, dec_mm, dec_ss" 
      write(39,*)2000," ! epoch_ref " 
     ...
     -                    nullval,specU,anyflg,status)
         ...
         do iy = ypix_beg,ypix_end,incs(2)
            ...
            write(39,*)(data_arrQ(i),i=1,nz_out),
     -                 (data_arrU(i),i=1,nz_out)
            !----------------------------------------------------
            !----------------------------------------------------
            ngood_chan = 0
            cnt2 = nz_out + 1
