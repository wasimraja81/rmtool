chelp+
      !---------------------------------------------------------
      ! This code extracts single cuts QU data from the FITS 
      ! IMAGE CUBES. The output is written out in a format 
      ! suitable for tomography codes developed for the purpose 
      ! of Pulsar Tomography. 
      !                -- wasim raja, 24 June, 2011
      !---------------------------------------------------------
chelp-
      ...
      real*4    data_cubeQ(max_ra,max_dec,maxchan),
     -          data_cubeU(max_ra,max_dec,maxchan)
      ...
      real*4 I_now(maxchan),Q_now(maxchan),
     -       U_now(maxchan), V_now(maxchan)
      real*4 BW_MHz, fcen_MHz
      ...
      integer*4 rwmode
      character*172 infileQ, infileU, message
      character*172 outfile, linecut_file
      character*172 subim_parfile, cfgfile
      character*172 path
      character*1 yorn
...
102   continue
      ...
      linecut_file(1:) = outfile(1:nchar(outfile))//'.QU.linecut'
...
602   continue
     ...
     -           cubeQ,message,status)
      ...
      if (status.eq.0)then
              ...
              write(*,*)"with the Q-cube file: ",
     -                         infileQ(1:nchar(infileQ))
     ...
     ------------" 
      ...
      ! part of them (In this code we are indeed reading part 
      ! of the cube -- only a line-cut)
...
501   continue
     ...
     -      nx_totpix, ny_totpix,nz_totpix, data_cubeU, anyflg, status)
      ...
      write(*,*)"Writing the data out to:",
     -                linecut_file(1:nchar(linecut_file))
      !----------------------------------------------------
      ...
      enddo
      if(.not.MHz)then
              BW_MHz = (zval(zpix_end) - zval(zpix_beg))/1.0e6
              fcen_MHz = 0.5*(zval(zpix_end) + zval(zpix_beg))/1.0e6
      else if(MHz)then
              BW_MHz = (zval(zpix_end) - zval(zpix_beg))
              fcen_MHz = 0.5*(zval(zpix_end) + zval(zpix_beg))
      else
              write(*,*)"Frequency units could not be established!"
              write(*,*)"Please check the headers in the input "
              write(*,*)"FITS files and manually modify the conv_fac"
              write(*,*)"in this code..."
              write(*,*)"Quitting now..."
              stop
      endif
      ...
      !---------------------------------------------------
      open(unit=81,file=linecut_file,status='unknown')


      if(nx_out .eq. 1)then 
              icnt = ny_out
      else if(ny_out .eq. 1)then 
              icnt = nx_out
      else
              write(*,*)"This code only extracts a line-cut!"
              write(*,*)"Please reconfigure your cfgfile!"
              write(*,*)"Quitting now..."
              stop
      endif

      write(81,*)1,icnt,4,nz_out ! npul,nbins,nstk,nchan
      write(81,*)fcen_MHz,BW_MHz,0.0,0.0    ! fcen_MHz, BW_MHz,P0,DM

      write(81,*)'Cal' ! source-name
      write(81,*)'GMRT' ! site-name
      write(81,*) '2011 04 18 21 50 39 '
      write(81,*)'00 00  00.00' ! ra: hh mm ss.ss
      write(81,*)'00 00  00.00 ' ! dec: dd mm ss.ss
      write(81,*)'2000.'  ! reference-epoch
      write(81,*)' # Bad-Chan Flags: ' ! comment line: "# Bad-Chan Flags: "
      write(81,*)(flag_arr(i),i=zpix_beg,zpix_end,incs(3))
      write(81,*)'# Bad-Chan Flag detection done:' !comment line: "# Bad-Chan Flag detection done: "
      !----------------------------------------------------
      ! Either we have requested a cut across constant RA(x) 
      ! or cut across aconstant Dec (y): 
      if(nx_out .eq. 1)then
              ix = xpix_beg
              do iy = ypix_beg,ypix_end,incs(2)
                 ...
                 do i = zpix_beg,zpix_end,incs(3)
                    icnt = icnt + 1
                    I_now(icnt) = 1.0
                    Q_now(icnt) = data_cubeQ(ix,iy,i)       
                    U_now(icnt) = data_cubeU(ix,iy,i)
                    V_now(icnt) = 0.0
                 enddo
                 write(81,*)(I_now(i),i=1,nz_out),
     -                      (Q_now(i),i=1,nz_out),
     -                      (U_now(i),i=1,nz_out),
     -                      (V_now(i),i=1,nz_out)
              enddo     ! end of iy loop
              close(81)
      elseif(ny_out .eq. 1)then
              iy = ypix_beg 
