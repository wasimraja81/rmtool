module commonbeam_mod
   ...
   !! shape, so the common beam we want is, by the physics of
   ...
   !! for performance, to avoid feeding tens of thousands of
   ...
   !! verify the candidate common beam can be deconvolved from
   ...
   !! matters, independently confirmed deconvolvable from
...
contains
   ...
   subroutine convex_hull_2d(n, x, y, hull_x, hull_y, nhull)
      ...
      !! dominates); done to shrink the point count the Khachiyan
   ...
   subroutine min_vol_ellipse(n, x, y, tolerance, amat, status)
      ...
      !! radio_beam's own numpy code does; harmless there
      ...
      !! because it solves the general problem of fitting an
      ...
      !! beam we want IS, by the physics of the problem, the
      ...
      !! match to what this module needs AND simpler: no
   ...
   subroutine deconvolve_is_valid(maj1, min1, pa1_rad, maj2, min2, pa2_rad, ok)
      ...
      !! to check "is this candidate common beam big enough for
      !! every real input beam", not to perform a
      !! deconvolution.
