module rmclean_mod
   ...
   ! provable property -- get_lsq_ref_compute's own comment)
   ...
   ! lsq_ref_compute value is in use, regardless of mode.
...
contains
   ...
   &re_peak, im_peak)
      ...
      !! interpolated complex value (amplitude AND phase) instead
      !! of magnitude alone.
      !!
      ...
      ! over, a domain violation (log of zero is mathematically
   ...
   &cos_arr, sin_arr, nrm, maxrm, maxchan, lsq_ref_compute, re_beam, im_beam)
      ...
      !! get_lsq_ref_compute(mode=centroid) is the example of a
      ...
      !! way, it introduces a phase distortion that differs
   ...
   &native_ddelta, oversample, table)
      ...
      !! compute_dirty_rmbeam_direct's own comment on why this is a
   ...
   &re_at_peak, im_at_peak, used_search)
      ...
      !! that dependency: it never interpolates the stored
      ...
      !! over-determined, and its leftover misfit is a
      !! self-consistency diagnostic -- small means the single-component
      !! model at the parabola's location does explain the
   ...
   &trace_flux_val)
      ...
      !!    of the peak value. phase_val/2 is used here, not
      !!    phase_val.
      !!
      ...
      !! across iterations), but a problem for anything that needs
      ...
      !! the bottleneck was never insufficient global grid
      ...
      !    pixel's own peak amplitude drops to or below a fixed
      ...
      !    before the peak has reached the noise floor,
      ...
      !    vs true) -- than using the full spectrum's own IQR
      ...
      ! stop_reason: which of the three criteria fired --
      ...
      ! fire on the last allowed iteration").
      ...
      ! actually run, index 1..n_iter_used): mechanical data
      ...
      ! caller a "pre-CLEAN" data point at iter=1 for free.
      ...
      ! iterations to resolve the source -- suspiciously close to
      ...
      ! in practice -- but it is an identifiable precision
      ...
      do iter = 1, niter
         ...
         ! why each is a separate, unambiguously-named
   ...
   subroutine estimate_iqr_sigma(dirty_re, dirty_im, nrm, sigma)
      ...
      !! own top comment has the full story on why that was a bug,
   ...
   subroutine compute_rmsf_fwhm(l_sq, nchan, fwhm_rm)
      ...
      !! pi and 2*sqrt(3) are close enough that this was never the
      !! question). Uses this module's own c_velocity (299.792458,
      !! matching rm_synthesis_mod.f90's exactly) rather than the
      !! original code's rounded "300.0" -- a precision improvement,
      ...
      !! returns, so the extra 0.5 here silently halved the
      ...
      !! and grid-spacing (get_drm, a different bound) never use
      ...
      !! compute_dirty_rmbeam and every test in this project
   ...
   &n_bands, fwhm_rm)
      ...
      !! band's own channel coverage. The thesis's own text (Sec
   ...
   subroutine get_drm(l_sq, nchan, lsq_ref_compute, drm, oversample)
      !! The RM-grid spacing to use, GIVEN a chosen
      !! lsq_ref_compute -- a sampling-theorem bound, not an
      ...
      !! -- unlike lsq_ref_compute/lsq_ref_report (free choices,
      ...
      !! velocity/envelope, which carries the information content
      ...
      !! complex beam every iteration) -- it needs the complex
      !! VALUE, not just the magnitude, so it cannot discard
      ...
      !! oversample=1 (tests/test_drm_floor.f90), an error, not
      ...
      !! no longer describes what clean_complex needs. The
   ...
   &native_ddelta, table_oversample_user) result(table_oversample_eff)
      ...
      !! only when the floor exceeds it.
   ...
   subroutine get_lsq_ref_compute(l_sq, nchan, mode, lsq_ref_compute, fixed_value)
      ...
      !! against whichever lsq_ref_compute value results,
      !! regardless of mode) but can cost GRID SIZE (see below).
      !!
      ...
      !! question -- the achievable statistical precision -- but
      !! that is a property of the DATA, decoupled from this
   ...
   &re_in, im_in, re_out, im_out)
      ...
      !! how the computation was done. lsq_ref_report=0.0
      ...
      !! intercept and slope) -- a DIFFERENT optimization
      ...
      !! comp_rm_refined(j) as rm_samp(j) (the continuous location,
      ...
      !! satisfied exactly regardless of which lsq_ref_compute the
   ...
   subroutine get_lsq_ref_report(l_sq, nchan, mode, lsq_ref_report, fixed_value)
      ...
      !! alternative, a DIFFERENT optimum from get_lsq_ref_
      ...
      !! wts_gpu, a per-PIXEL, per-CHANNEL 0/1 mask, confirmed by
      ...
      !! a graduated (non-0/1) weight, if this project ever
