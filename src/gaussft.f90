module gaussft_mod
   ...
   !! ASKAP data is partially blanked (primary-beam edge
   ...
   !! or one of many from several bands; "multi-band" is a matter
   ...
   !! unchanged since this module's first version.
   !!
   ...
   ! write_matched_file's own FTPHPR calls), so there is no
   ...
   ! only cast down to sp at the point g_final is stored --
   ...
   ! stay real(dp) -- the precision reduction is internal to
...
contains
   ...
   subroutine plan_convolution(nx, ny, plan_fwd, plan_bwd, nx_pad, ny_pad, nthreads)
      ...
      ! it -- see below): every convolve_to_beam call uses the
      ...
      ! fixes MEASURE's own cross-invocation non-determinism
      ! (found via a real test failure: io_overlap=y vs n, two separate
      ! process invocations of the identical transform, differed by
      ! ~1e-7 relative -- MEASURE times candidate decompositions
      ! empirically, and that timing is subject to real-world jitter between
   ...
   function next_fast_fft_size(n) result(m)
      ...
      !! hard-coded, just the case that exposed the problem): an
   ...
   &status)
      ...
      do iy = 1, ny_pad
         do ix = 1, nx_pad
            ...
            ! single precision here would risk cancellation error);
