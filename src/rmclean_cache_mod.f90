module rmclean_cache_mod
   ...
   !! rmclean_cubes.f90's own program-internal cache logic into a
   ...
   type :: pattern_registry_entry_t
      ...
      !! before CLEAN compute starts, then consulted (never
      !! rebuilt) during the run to answer Belady's own question:
      ...
      !! run regardless of whether this pattern is currently
...
contains
   ...
   subroutine registry_lookup(reg, pattern, n, entry_id)
      ...
      !! appends an occurrence. Used during Pass 1 to
      ...
      !! as a scan-order-mismatch bug, not a normal case to
      !! silently handle.
   ...
   subroutine registry_advance(reg, entry_id)
      ...
      !! position -- called once per pixel visit during the run
