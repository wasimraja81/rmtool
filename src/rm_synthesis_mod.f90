module rm_synthesis_mod
  ...
  ! parser's own case-statement clusters, for readability -- it
  ...
  type :: rmsynth_config_t
    ...
    ! those scalars unchanged, for any band count.
  ...
  ! the /proc/meminfo read itself (file I/O) and passes the resolved
  ...
  ! independent of the OpenMP runtime, so it can run concurrently
  ...
  type :: tile_write_job_t
    ...
    ! path_pha, bypassing CFITSIO for the pixel data -- see
...
!$omp end critical (logger_write_lock)
  ...
  subroutine extract_general_setup(t, npts, fac, beg_rm, end_rm, nout, nu, cos_arr, sin_arr, maxout, maxpts, use_auto_rm_range, ofac, lsq_ref)
    ...
    ! freq_MHz(1)/freq_MHz(npts) are the dataset's true min/max
    ...
    ! does NOT affect the DFT template build below (the cos_arr/
  ...
  subroutine plan_tile(plan)
    ...
    plan%tile_dec = plan%tile_dec_in
     ...
     ! subdividing RA (rare; wide images).
  ...
  ! bypassing CFITSIO's handle/buffer machinery for the pixel
  ...
  logical function host_is_big_endian() result(is_be)
    ...
    !! case" would be a portability bug, not just a missed
    !! optimisation.
    implicit none
    integer(int32) :: probe
  ...
  subroutine write_rm_chunk_raw(file_path, datastart, nx_out, ny_out,&
    ...
    !! pixel data -- see the module-level comment above for why.
    !!
    ...
    ! own `!$omp parallel do`, and a first attempt at fixing an
    ! observed corruption here (docs/dev/RMCLEAN_INTEGRATION_PLAN.md
    ...
    ! itself atomic instead: unique (real newunit= semantics,
    ! no manual range to maintain), while the write below still
    ! runs in parallel per thread -- only the brief "grab a unit"
  ...
  &rm_peak_tile_arr, ang_peak_tile_arr, snr_tile_arr)
    ...
    !! stay untouched, in their exact original order, in the main
  ...
  subroutine do_tile_write(job)
    ...
    ! calls below them, so this total tracks exactly what hits
    ...
    ! CFITSIO's ftpsse/handle machinery for the pixel data -- see
    ! that routine's header comment for why this is what makes
