...
contains
  ...
  subroutine tile_extract_gpu(specQ, specU, cos_arr, sin_arr, &
    ...
    !!
    !! Weight-based channel masking (unified architecture):
    !!   - Templates are FULL-SIZE: cos_arr(nz_out, nrm_out), sin_arr(nz_out, nrm_out)
    !!   - All nz_out channels have valid cos/sin values pre-computed
    !!   - Bad channels are masked via wts_tile=0 (both global and per-pixel bad channels)
    !!   - DFT uses direct indexing: cos_arr(cnt2, i) for channel cnt2, no kk mapping needed
    !!
    !! Masking sources:
    !!   - Globally-bad channels (flag_arr==0): Q=0, U=0, wts=0 (zeroed in data load)
    !!   - Per-pixel bad (NaN/mask): Q=data, U=data, wts=0 (weighted by 0 in sums)
    !!   - Good channels (flag_arr==1 AND per_pix_valid): Q=data, U=data, wts=1
    !!
    !! ngood_tile(ipix) stores n_chan_tmpl (= nz_out) for line-cut writes;
    !! nvalid_tile_arr stores per-pixel count of actually valid (wts>0) channels.
    !!
    ...
    !$omp     map(to: specQ, specU, mask_tile_arr,                   &
    !$omp             cos_arr, sin_arr,                               &
    ...
    do ipix = 1, nx_tile*ny_tile
      ...
      ! --- Single pass: load data and apply unified mask ---
      ! mask_tile_arr already contains all masking info:
      !   - Global bad channels (from bad_chan list)
      !   - NaN/Inf detection on Q and U
      !   - Per-pixel mask FITS (if provided)
      ...
      do cnt2 = 1, nz_out
        ...
        if (mask_tile_arr(tmp_index) == 1) then
          ...
          ! Store data with full weight
          Q_tile(tmp_index) = q_now
          U_tile(tmp_index) = u_now
          wts_tile(tmp_index) = 1.0_sp
          if (nvalid_pix < int(nz_out, kind=int32)) nvalid_pix = nvalid_pix + 1
        else
          ! Globally bad channel: zero everything (no uninitialized garbage)
          Q_tile(tmp_index) = 0.0_sp
          U_tile(tmp_index) = 0.0_sp
          wts_tile(tmp_index) = 0.0_sp
        end if
      end do
      ...
      if (wsum <= 0.0_sp) then
        do i = 1, nrm_out
         ...
         ! Compute per-pixel means over all channels (wts_tile=0 handles masking)
         mean_q = 0.0_sp
         mean_u = 0.0_sp
         if (rem_mean > 0) then
           do cnt2 = 1, nz_out
         ...
         do i = 1, nrm_out
           ...
           do cnt2 = 1, nz_out
             ...
             rs_cor = rs_cor + wts_tile(tmp_index) * q_eff * sin_arr(cnt2, i)
             ic_cor = ic_cor + wts_tile(tmp_index) * u_eff * cos_arr(cnt2, i)
             is_cor = is_cor + wts_tile(tmp_index) * u_eff * sin_arr(cnt2, i)
           end do
           rc_cor = rc_cor / wsum
           rs_cor = rs_cor / wsum
           ic_cor = ic_cor / wsum
