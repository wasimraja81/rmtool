...
contains
   ...
   &crpix_shift2, nx_out, ny_out) result(matches)
      ...
      !! different sky positions), so every comparison is a
      !! tight absolute tolerance, and a file that is close but
   ...
   subroutine get_matrix_2x2(unit, axis1, axis2, m, have_any)
      !! Read the 2x2 PCi_j (falling back to CDi_j if PC is
