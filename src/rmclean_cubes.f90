...
! which channels were valid there).
!
...
! computation uses -- IS a free, independent config choice
...
! 'native' here, a drift from that recommendation, not a
! deliberate departure; fixed together with this comment). Choosing a
! DIFFERENT value is supported: derotate_to_lsq_ref is an exact,
...
! matched-filter template -- a mismatch is a phase distortion, not
...
program rmclean_cubes
   ...
   ! MAY be combined (not mutually exclusive -- a design change
   ...
   ! drops to/below this fixed flux value -- accepts a bare
   ...
   ! zero components gets an explicit 0.0 (an empty sum is a
   ...
   ! avoids a separate no-overlap code path.
   ...
   ! parallel do` regions undisturbed by the write's own
   ...
   ! BLOCK, not only once at the end. Motivated by the mosaic-edge
   ...
   ! extracted into components, run-wide. peak_residual_*: each
   ...
   ! run_pattern_prescan before compute starts, consulted (never
   ! rebuilt) by the Belady eviction decision during the run.
   ! Allocated/populated only when cache_eviction_policy='belady'; left
   ! untouched (registry%n_entries stays 0) under 'hitcount',
   ...
   ! restoring beam, not what the data can resolve).
   ...
   ! down, after init_mask_pattern_cache) so Pass 0 and the Pass-1
   ...
   ! a pixel's own real value when it reaches a normal
   ...
   ! Each out_unit(idx) is assigned its own distinct CFITSIO
   ...
   ! ampfile's own amplitude BUNIT instead, a pre-existing bug found
   ...
   ! CLEAN/RESID take no rmsffwhm_override (only RESTORED has
   ...
   ! T14 increment 9: direct, printed evidence for the point of
...
contains
   ...
   subroutine resolve_abs_flux_floor(status)
      ...
      !! number used, not just the cfg's own request.
      integer, intent(out) :: status
   ...
   subroutine plan_rmclean_tile()
      ...
      !! run per RM-plane; only a wide image (a single full-RA
      ...
      ! divides by). 21 = 2(NITER)+1(STOP_REASON)+4(RESID_PEAK)
   ...
   &im_out, status)
      ...
      !! opening its OWN readonly FTOPEN handle (a distinct unit
      ...
      !! of). Separately, and regardless of same-file-ness: the
   ...
   &rm_beg, rm_len, thread_id, re_out, im_out, status_par)
      ...
      !! would have been the first to hit that crash under
   ...
   &n_template_cards, status)
      ...
      !! undo sparseness, even after CFITSIO header work (FTUKYS/
      ...
      !! header_rmclean copies every card from the ampfile
      ...
      !! status)'s own defaults exactly, pcount=0/gcount=1/no
      !! EXTEND card since extend=.false.), then blank filler cards,
      !! then END at the last reserved card slot, then sets the
      !! file's final logical size via a single trailing-byte stream
      !! write (sparse on this project's output filesystem,
      ...
      ! (already initialized above) -- END goes at the last card
   ...
   &idx, lsq_ref_report_out, status, bunit_override, rmsffwhm_override)
      ...
      !! reference this output's own pixel data is derotated to,
      ...
      !! convolved into. MUST be applied here, inside this
      ...
      ! below (not guessed; see that subroutine's own comment on
      ...
      ! otherwise operate on a unit CFITSIO never set up.
      ...
      if (present(bunit_override)) then
         ...
         ! FTPREC card copy just above -- confirmed directly (a
         ...
         ! for and updates an existing keyword (inserting only if
         ! absent), which is what "override" requires here.
         fitsstat = 0
   ...
   &ix0, iy0, tx, ty, re_in, im_in, status)
      ...
      !! CFITSIO's ftpsse for the pixel data, since N CFITSIO
   ...
   subroutine do_tile_write(job)
      !! T4d: the "write one tile's 6 output cubes" payload --
   ...
   &rm_end, data)
      ...
      !! pixel data (see write_output_tile's own comment for
      !! why).
      !!
      ...
      !! unique (real newunit= semantics, no manual range to
      !! maintain), while the write itself below still runs in
   ...
   subroutine copy_generic_header_rmclean(src_unit, dst_unit, status)
      ...
      do i = 1, nkeys
         ...
         ! is NOT generally what this program's own outputs are
   ...
   subroutine write_diagnostic_maps()
      ...
      !! no tiling/chunking needed unlike the 6 (3D) outputs.
   ...
   function cached_pattern_matches(cidx, pattern) result(matches)
      ...
      !! since WHERE that pattern's bytes live depends on
   ...
   subroutine cache_lookup_readonly(pattern, entry_idx)
      ...
      !! update_mask_pattern_cache_for_tile has already populated
      ...
      !! not a valid one). Only an empty slot (0) proves "not
      !! cached, stop looking."
      integer(kind=1), intent(in) :: pattern(:)
   ...
   subroutine run_pattern_prescan()
      ...
      !! WHOLE mask cube once, before CLEAN compute starts,
   ...
   subroutine report_build_time_advisory()
      ...
      !! CLEAN-iteration time, which depends on each pixel's own
      ...
      !! recoverable). Printed to stdout only -- already
      ...
      !! time by ~7x because the real 4096-slot cache is far
      !! smaller than that block's own local diversity, forcing
      ...
      !! asked-for question directly from data already collected:
      ...
      !! occurrence is huge() (never again, unique data no
      ...
      !! post-Pass-0 state for Pass 1's own use) -- every
      ...
      !! cost Pass 1 itself pays for its own real eviction
      ...
      do
         ...
         do iy_l = 1, ty
            do ix_l = 1, tx
               ...
               ! below for a reason that has nothing to do with a
               ! bug.
               if (min_valid_chan_frac.gt.0.0_sp .and.&
               &real(count(mask_tile(ix_l,iy_l,:).ne.0_1), sp).lt.&
               &min_valid_chan_frac*real(nchan, sp)) cycle
   ...
   subroutine evict_cache_slot(victim_idx)
      ...
      !! slot whose value equals victim_idx -- correct and
   ...
   subroutine update_mask_pattern_cache_for_tile(tx_in, ty_in)
      ...
      !! reset here).
      !! T14: once the cache is full, a new pattern either
      ...
      !! not cached. Either way, target_slot is where the new
      !! pattern's own table gets built -- a NEW cache row
      !! (n_cache_entries incremented) or an EVICTED, now-reused one;
      !! the insert logic below is identical either way.
      ...
      do iy_l = 1, ty_in
         do ix_l = 1, tx_in
            ...
            ! will be skipped in clean_one_pixel too (which
            ...
            if (n_cache_entries.ge.mask_pattern_cache_max) then
               if (cache_eviction_policy.eq.'hitcount') then
                  ...
                  ! T14 increment 8: 'belady', cache full -- the
            ...
            ! Bucket slot for target_slot: reuse either an
   ...
   subroutine clean_one_pixel(ix_l, iy_l, ix0, iy0)
      ...
      !! addressing. Every array declared here is a LOCAL
      ...
      !! thread-safe by construction, no explicit `private()`
      ...
      if (lsq_ref_compute.ne.lsq_ref_native) then
         ...
         ! spectrum from the reference it was built at
      ...
      ! module for these two) -- total flux CLEAN extracted
      ...
      if (lsq_ref_report.ne.lsq_ref_compute) then
         ...
         ! lives AT rm_samp(j) after smoothing) -- rm_samp is the
      ...
      ! resid_re_p/resid_im_p about to be written to the outputs
      ! below) for consistency; amplitude-based quantities are
      ...
      if (write_clean_diagnostics) then
         ...
         block
            ...
            ! reasoning applies here: a 2-line utility, not
         ...
         block
            ...
            ! conflates genuine multi-screen structure with CLEAN
            ...
            ! array is a correct 0.0, not an undefined case.
