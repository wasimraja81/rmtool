...
!help-
 ...
 ! is memory efficient, may be inefficient in time.
...
use thread_safety_mod, only: check_and_clamp_omp_threads
implicit none
...
call check_and_clamp_omp_threads()
...
if (trim(cfgfile).eq.'--help' .or. trim(cfgfile).eq.'-h') then
   call print_usage()
   stop
endif
...
! been threaded out by T5 -- every field is now read as cfg%field
...
if (n_bands_t2.gt.1)then
   ...
   ! remain out of scope -- every one stops here, explicitly,
   ...
   ! code below, unchanged, writing into L_sq(1:nz_out) as it
 ...
 !       the axes are the central pixels or not.
 ...
 ! Check if the reference pixel is at the centre of the
...
if(cfg%remove_badchan)then
   ...
   ! band has nothing to list -- skip the open
   ...
   ! further down), which is correct for an open failure but
   ! would be wrong here -- 'none' for THIS band must never suppress a
   ! different band's own bad-channel file.
...
nbuffer = naxes(1)
 ...
 ! several READONLY handles on the same file -- not the exact
...
ntot_out = nx_out*ny_out*nz_out
 ...
 ! Allocate per-pixel spectrum work buffers sized to nz_out
...
if (n_bands_t2.gt.1) then
   do iband = 1,n_bands_t2
      ...
      if (cfg%remove_badchan) then
         ...
         ! this band has nothing to list -- skip open/allocate
...
if (n_bands_t2.gt.1) then
   ...
   do iband = 1,n_bands_t2
      ...
      &band_czval(iband).le.10.0e9)then
         ...
         ! reached with a confusing magnitude; this branch is
...
if(need_icube)in_fields = 3
 ...
 ! /proc/meminfo just above -- that read is file I/O and stays a
...
plan%mem_frac_vram = cfg%mem_frac_vram
plan%gpu_vram_mib = cfg%gpu_vram_mib
plan%tile_ra_in = cfg%tile_ra
plan%tile_dec_in = cfg%tile_dec
plan%tile_auto = cfg%tile_auto
...
cfg%tile_ra = plan%tile_ra
cfg%tile_dec = plan%tile_dec
...
write(*,*)" mem_frac_ram: ",cfg%mem_frac_ram
write(*,*)" tile_ra x tile_dec (RAM read px): ",cfg%tile_ra,cfg%tile_dec
write(*,*)" Estimated RAM block memory (MB): ",&
&real(tile_bytes_est,kind=dp)/(1024.0_dp*1024.0_dp)
write(*,*)" mem_frac_vram: ",cfg%mem_frac_vram
...
write(*,*)" VRAM sub-block per slot (px): ",cfg%tile_ra,ny_sub
...
if(cfg%dry_run)then
   ...
   write(96,*)"tile_ra=",cfg%tile_ra
   write(96,*)"tile_dec=",cfg%tile_dec
   write(96,*)"mem_frac_ram=",cfg%mem_frac_ram
   write(96,*)"mem_frac_vram=",cfg%mem_frac_vram
   write(96,*)"# inflight_slots_planned=",inflight_slots_planned
   write(96,*)"# mem_frac_vram_per_slot=",mem_frac_vram_per_slot
   write(96,*)"gpu_vram_mib=",cfg%gpu_vram_mib
...
ios_mem = 0
 ! Use int64 arithmetic for array sizes: cfg%tile_ra*cfg%tile_dec*nz_out can exceed
 ! INT32_MAX (e.g. 13308*734*288 = 2.81e9) causing silent 32-bit wrap and a
 ! misallocated buffer that FTGSVE then overruns.
allocate(specQ(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nz_out,kind=int64)),&
&specU(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nz_out,kind=int64)),&
&p_tile_arr_s0(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nrm_out,kind=int64)),&
&phi_tile_arr_s0(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nrm_out,kind=int64)),&
...
if(ios_mem.eq.0 .and. cfg%io_overlap)then
   ...
   &mask_tile_arr_s1(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nz_out,kind=int64)),&
...
if(ios_mem.eq.0 .and. cfg%cubestat)then
   ...
   if(ios_mem.eq.0 .and. cfg%io_overlap)then
      ...
      &snr_tile_arr_s1(cfg%tile_ra*cfg%tile_dec),&
      &stat=ios_mem)
   endif
endif
if(ios_mem.eq.0)then
...
if(ios_mem.eq.0 .and. use_input_mask)then
   allocate(specMask(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nz_out,kind=int64)),stat=ios_mem)
endif
if(ios_mem.eq.0 .and. need_icube)then
   allocate(specI(int(cfg%tile_ra,kind=int64)*int(cfg%tile_dec,kind=int64)*int(nz_out,kind=int64)),stat=ios_mem)
endif
...
if(ios_mem.eq.0 .and. use_staging)then
   allocate(stQ(cfg%tile_ra*ny_sub*nz_out,2),&
   &stU(cfg%tile_ra*ny_sub*nz_out,2),&
   &stP(cfg%tile_ra*ny_sub*nrm_out,2),&
   &stPhi(cfg%tile_ra*ny_sub*nrm_out,2),&
   &stMaskOut(cfg%tile_ra*ny_sub*nz_out),&
   &stMask_tile_arr(cfg%tile_ra*ny_sub*nz_out,2),&
   &stNvalid(cfg%tile_ra*ny_sub,2),&
   &stat=ios_mem)
   if(ios_mem.eq.0 .and. use_input_mask)then
      allocate(stMask(cfg%tile_ra*ny_sub*nz_out,2),&
      &stat=ios_mem)
   endif
   if(ios_mem.eq.0 .and. need_icube)then
      allocate(stI(cfg%tile_ra*ny_sub*nz_out,2),&
      &stat=ios_mem)
   endif
endif
...
if(ios_mem.ne.0)then
   ...
   write(*,*)" tile_ra x tile_dec: ",cfg%tile_ra,cfg%tile_dec
...
call ftpkyd(42,'cdelt3',dble(dRM),decimals,'RM spacing',status)
 ! LSQREF: the phase-reference lambda^2 (m^2) used to build
...
if(status.eq.0)then
 ...
 ! function F(phi) is a Faraday-depth DENSITY, not a plain
...
if(out_snr_open)then
 ...
 ! error -- not written to the outputs either, same as this
 ...
 ! meaningful for a multi-band RM cube in the first place.
status = 0
...
status = 0
 ...
 ! gets CASAMBM=T plus the INPUT's own per-channel BEAMS table,
 ...
 ! from the flux-bearing Q/U data, so "which beams went into this"
 ! is a useful provenance question for all of them, even though
 ! none of their own axes directly correspond to input channels (AMP/
 ! PHA's 3rd axis is Faraday depth; the maps are 2D, collapsed
...
if(casambm_status.eq.0 .and. casambm_val)then
   ...
   if(out_snr_open)then
    ...
    ! own header comment on the same lesson) -- opened to read the
   ...
   if(beams_status.eq.0)then
       ! CASAMBM is only written alongside an attached BEAMS
...
status = 0
 ...
 ! correct and complete for single-band runs, but a gap for
 ...
 ! CASAMBM=T, unreflected by the single value already written
 ! above. RM synthesis' computation does not depend on any of
 ! this -- combining bands with different resolutions is not a
 ...
 ! hard error (this project already hard-stops on geometry
...
if(use_input_mask)then
 ...
 ! are clamped to what the runtime can deliver.  In serial builds
...
! writes only the LOWER 32 bits of FTGHAD's 3 output arguments (a
...
! the stack), which is why this call site was never observed to
...
do ix_tile_beg = xpix_beg,xpix_end,cfg%tile_ra*incs(1)
   ix_tile_end = min(xpix_end,&
   &ix_tile_beg + (cfg%tile_ra-1)*incs(1))
   nx_tile = int((ix_tile_end - ix_tile_beg)/incs(1)) + 1

   do iy_tile_beg = ypix_beg,ypix_end,cfg%tile_dec*incs(2)
      iy_tile_end = min(ypix_end,&
      &iy_tile_beg + (cfg%tile_dec-1)*incs(2))
      ...
      ! and the per-thread split below come from the same numbers.
...
!$omp end parallel do
      ...
      if(cfg%io_overlap)then
         ...
         ! RM-chunks already write to disjoint byte ranges via
...
if(allocated(par_unit_I))    deallocate(par_unit_I)
 ...
 ! binary-table-extension pattern above. This is a prerequisite
 ! for RM-CLEAN (docs/dev/RMCLEAN_INTEGRATION_PLAN.md): a standalone
 ! clean tool consuming a saved dirty AMP/PHA cube has no other reliable
 ! way to recover each channel's true lambda-squared.
 !
 ! Written from L_sq/flag_arr_out/band_offset, which are still
...
subroutine print_usage()
   write(*,'(A)') 'rm_synthesis -- RM synthesis (lambda^2 -> Faraday depth) on Stokes Q/U cubes'
   write(*,'(A)') ''
   write(*,'(A)') 'Usage:'
   write(*,'(A)') '  rm_synthesis <cfgfile>'
   write(*,'(A)') '  rm_synthesis --help | -h'
   write(*,'(A)') ''
   write(*,'(A)') '<cfgfile> is a plain-text KEY=VALUE config file; every run parameter'
   write(*,'(A)') 'comes from it, there is no command-line key=value override. Resolved'
   write(*,'(A)') 'either as given, or as ../cfg/<cfgfile> if not found directly.'
   write(*,'(A)') ''
   write(*,'(A)') 'cfg/rmsynth.cfg is an annotated template config to copy and adapt.'
   write(*,'(A)') 'docs/user/APP_REFERENCE.md documents every key, its default, and'
   write(*,'(A)') 'every output file this tool writes.'
