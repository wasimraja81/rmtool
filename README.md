...
Clone the repository, then run everything below from inside it (these
docs call that directory `$RMTOOL_HOME` — see
[QUICKSTART.md](QUICKSTART.md#where-things-go) for the full
convention):

```bash
git clone https://github.com/wasimraja81/rmtool.git ~/rmtool && cd ~/rmtool
./build.sh                                          # checks dependencies, builds everything
bin/rm_synthesis_release_cpu_omp cfg/rmsynth.cfg     # run it
```
...
[docs/user/TUTORIAL.md](docs/user/TUTORIAL.md). For the same thing run
end to end in two commands, plus fetching your own data, see
...
See [QUICKSTART.md](QUICKSTART.md#want-to-try-it-on-real-data) for
the free-account setup and full detail.
